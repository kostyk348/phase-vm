...
| дробные классы размеров (шаг 16..512) | mixed 246 ns/op (хуже 148), RSS 8.2 МБ | откачено |

**Итоговый профиль характера аллокатора** (после серии замеров):
- кадровая нагрузка (96M alloc, размеры 16..4 КБ): паритет с glibc (2.6 vs 2.4 ns/op), RSS на паритете;
- adversarial mixed (16 Б..1 МБ, 8 потоков): мы ~3.5x медленнее glibc (260 vs 62-80 ns/op),
  НО RAM **7.8 МБ против 117-125 МБ (≈15x меньше)** — здесь наша ниша;
- загрузка реальной игры (CW2): +6..+14% к glibc;
- HOI4: запускается (уникально для malloc-preload);
- DIARY: append-only, free=надгробие, rewind O(1).

Вывод: инкрементальный тюнинг насытился (4 гипотезы отвергнуты замером).
Дальнейший рост скорости на mixed требует повторить tcache-подход glibc
(per-thread кэши без реестра) — ценой части RAM-выигрыша.
| -O3, pending-счётчик, удаление owner-store | 51.9/57.0/178 cyc против 46.1/52.6/160.7 — в пределах шума (±15%) | откачено |

Примечание о шуме: одна и та же сборка давала malloc 46 и 53 cyc,
mixed 132 и 269 ns/op в разных прогонах (загрузка CPU). Миктотюнинг достиг
уровня шума — дальнейшие ±10% неотличимы на этой машине.
