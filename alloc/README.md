...
произвольных больших приложений**. Открытый дефект: CPython 3.14 (2+ потоков,
интенсивные list/bytes аллокации) под LD_PRELOAD виснет (self-deadlock/спин на
g_lock в phase_free, воспроизводится /tmp/light2.py). Root-cause не завершён.
Для Steam-игр прелоад ПОКА НЕЛЬЗЯ — нужна итерация на конкретной программе.
Проверено:
